<template>
  <div class="main" v-if="visible">
    <div id="text" v-for="({ data }, index) in data" :key="index">
      <router-link to="">
        <span id="title" v-html="data.title"></span>
        <span id="subtitle" v-html="data.subtitle"></span>
      </router-link>
      <div id="timer" v-if="data.enable_timer && showTimeInHours">
        Ends in {{ timeLeft.hours }}h {{ timeLeft.minutes }}m
        {{ timeLeft.seconds }}s
      </div>
      <div id="timer" v-else>{{ humanReadableTimeDiff }} left!</div>
    </div>
    <div id="icon" @click="closeNotification">
      <CloseOutlined />
    </div>
  </div>
</template>
...
const fetchAPi = async () => {
  try {
    const response = await axios.get(
      "https://www.udemy.com/api-2.0/notices/me/?type=smart_bar&limit=1"
    );
